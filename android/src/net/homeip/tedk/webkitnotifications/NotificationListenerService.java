...
public class NotificationListenerService extends AccessibilityService {
	...
	private static final Location work = new Location(LocationManager.NETWORK_PROVIDER);
	private static final Location home = new Location(LocationManager.NETWORK_PROVIDER);
	static {
	...
	protected void onServiceConnected() {
		...
		String authString = getResources().getString(R.string.server_username) + ":" + getResources().getString(R.string.server_password);
	...
	public void onAccessibilityEvent(AccessibilityEvent event) {
		...
		try {
			...
			BitmapFactory.decodeResource(c.getResources(), n.icon).compress(Bitmap.CompressFormat.PNG, 100, baos);
			baos.flush();
		...
		new SendNotification().execute(time, appName, text, num, icon, Integer.toString(iconPadding));
	}
